 ...
 *  played as a result of calls to {@link #play(double sample)},
 *  {@link #play(double[] samples)}, and {@link #play(String filename)}.
 *  <ul>
 ...
 *  <p>
 *  {@code StdAudio} does not currently support recording audio that is played in
 *  a background thread via calls to {@link #playInBackground(String filename)}.
 *  <p>
...
public final class StdAudio {
    ...
    // for recording audio
    private static ResizingArrayQueueOfDoubles recordedSamples = null;
    ...
    public static void startRecording() {
        if (!isRecording) {
            recordedSamples = new ResizingArrayQueueOfDoubles();
            isRecording = true;
        }
        else {
     ...
     ***************************************************************************/
    private static class ResizingArrayQueueOfDoubles {
        private double[] a;   // array of doubles
        private int n;        // number of items in queue

        // create an empty queue
        public ResizingArrayQueueOfDoubles() {
            a = new double[16];
            n = 0;
        }
